...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/chan/$id/json-schema"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/chan/${id}/json-schema`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
curl \
  ...
  --data "$JSON_SCHEMA" \
  "http://localhost:8080/admin/chan/$id/jsonschema"
#+END_SRC
...
****** 移除JSON Schema
=DELETE /admin/chan/<id>/json-schema=
...
******* 获取特定消息队列的token策略
=GET /admin/chan/<id>/token-policies=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/chan/$id/token-policies"
#+END_SRC

********* fetch
#+BEGIN_SRC js
await fethc(`http://localhost:8080/admin/chan/${id}/token-policies`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
******* 设置token策略
=PUT /admin/chan/<id>/token-policies/write-token-required=
=PUT /admin/chan/<id>/token-policies/read-token-required=
...
curl \
  ...
  "http://localhost:8080/admin/chan/$id/token-policies/write-token-required"
#+END_SRC

********* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/chan/${id}/token-policies/write-token-required`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  , 'Content-Type': 'application/json'
...
******* 移除token策略
=DELETE /admin/chan/<id>/token-policies/write-token-required=
=DELETE /admin/chan/<id>/token-policies/read-token-required=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/chan/$id/tokens"
#+END_SRC

********* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/chan/${id}/tokens`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
=PUT /admin/chan/<id>/tokens/<token>/write=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/chan/$id/tokens/$token/write"
#+END_SRC

********* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/chan/${id}/tokens/${token}/write`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=DELETE /admin/chan/<id>/tokens/<token>/write=
...
=PUT /admin/chan/<id>/tokens/<token>/read=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/chan/$id/tokens/$token/read"
#+END_SRC

********* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/chan/${id}/tokens/${token}/read`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=DELETE /admin/chan/<id>/tokens/<token>/read=
